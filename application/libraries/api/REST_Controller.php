...
abstract class REST_Controller extends CI_Controller {
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE)
    {
		...
		if(!$isProfilingEnabled){
            ...
            // JC: Log response  only if rest logging enabled
            if ($this->config->item('rest_enable_logging') === TRUE)
            {
                $this->_log_response_output($output);
            }
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        $is_inserted = $this->rest->db
            ->insert(
                ...
                'params'    => $this->{'_'.$this->request->method.'_args'} ? ($this->config->item('rest_logs_json_params') ? json_encode($this->{'_'.$this->request->method.'_args'}) : serialize($this->{'_'.$this->request->method.'_args'})) : null,
    ...
    function ParamValidation($paramarray,$data)
    {
        ...
        {
            $returnArr[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
            $returnArr[RESPONSE_MESSAGE] = 'Sorry, that is not valid input. You missed '.implode(',',$NovalueParam).' parameters';

        }
        else
        {
            $returnArr[RESPONSE_FLAG] = RESPONSE_FLAG_SUCCESS;
        }
        return $returnArr;
    }

    protected function _log_response_output($output)
    {
        if($this->_insert_id == ''){
            return false;
        }
