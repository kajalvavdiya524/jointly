 ...
 class User Extends REST_Controller
{
	...
	public function register_post()
	{
		...
		{
			$get_user = $this->db->get_where('tbl_user',array('business_phone_number'=>$input_data['mobile_no'], 'is_del'=>0))->row_array();
			if(!empty($get_user))
			{
				$response[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
				$response[RESPONSE_MESSAGE] = __MOBILE_ALREADY_EXIST;
			}
			else {
				$otp = rand(1000, 9999);
				//$otp = '1234';
				...
				$get_customer = $this->db->get_where('tbl_invite_customer',array('mobile_no'=>$input_data['mobile_no'],'is_del'=>0))->row_array();

				if(!empty($get_member)) {
					$role = 'team member';
				}
				elseif(!empty($get_customer)) {
					$role = 'customer';
				}
				else {
					$role = 'manager';
				...
				if($user_id)
				{
					...
					$get_referral2 = $this->db->get_where('tbl_new_referrer_info',array('referral_mobile_no2'=>$input_data['mobile_no'],'is_del'=>0))->result_array();
					if(!empty($get_referral2))
					{
						foreach ($get_referral2 as $referral2)
						{
							# code...
							$check_info_exist2 = $this->db->get_where('tbl_manage_referrer',array('info_id'=>$referral2['info_id'],'is_del'=>0))->result_array();
							if(!empty($check_info_exist2))
							{
								$this->mdl_common->update_record('tbl_manage_referrer',array('info_id'=>$referral2['info_id'],'is_del'=>0),array('referral_id2'=>$user_id));
							}
						}
					}
					$get_referral3 = $this->db->get_where('tbl_new_referrer_info',array('referral_mobile_no3'=>$input_data['mobile_no'],'is_del'=>0))->result_array();
					if(!empty($get_referral3))
					{
						foreach ($get_referral3 as $referral3)
						{
							# code...
							$check_info_exist = $this->db->get_where('tbl_manage_referrer',array('info_id'=>$referral3['info_id'],'is_del'=>0))->result_array();
							if(!empty($check_info_exist))
							{
								$this->mdl_common->update_record('tbl_manage_referrer',array('info_id'=>$referral3['info_id'],'is_del'=>0),array('referral_id3'=>$user_id));
							}
						}
					}
					...
					$get_team_member = $this->db->get_where('tbl_manage_team_member',array('mobile_no'=>$input_data['mobile_no'],'is_del'=>0))->row_array();
					if(!empty($get_team_member))
					{
						$agencyID = $get_team_member['user_id'];
						$manageReferre = $this->db->get_where('tbl_manage_referrer',array('referrer_id' => $user_id,'business_id'=>$agencyID,'is_del'=>0))->result_array();
						if(empty($manageReferre)) {
							$check_b_exist = $this->db->get_where('tbl_manage_referrer',array('business_id'=>$agencyID,'is_del'=>0))->result_array();
							foreach ($check_b_exist as $member)
							{
								$team_member_id = $user_id;
								if($member['team_member_id'] != '') {
									$team_member_id = $member['team_member_id'].','.$user_id;
								}
								$this->mdl_common->update_record('tbl_manage_referrer',array('info_id'=>$member['info_id'],'is_del'=>0),array('team_member_id'=>$team_member_id));
							}
						}
					}

					$userData = $this->mdl_common->get_record('tbl_user', Array('user_id' => $user_id, 'is_del' => 0))->row_array();
					$user_data1 = array(
						'user_id' => $user_id,
						'mobile_no'=>$input_data['mobile_no'],
						'is_verify'=>'0',
						'is_profile_complete'=>'0',
						'business_country_code' => $userData['business_country_code'],
						'business_phone_number' => $userData['business_phone_number'],
						'otp' => $userData['otp']
					);
		...
		{	
			...
			{	
				...
				// $get_member1 = $this->db->get_where('tbl_manage_team_member',array('mobile_no'=>$input_data['mobile_no'],'is_del'=>0))->row_array();

				// $get_customer1 = $this->db->get_where('tbl_invite_customer',array('mobile_no'=>$input_data['mobile_no'],'is_del'=>0))->row_array();

				// if(!empty($get_member1)) {
				// 	$role = 'team member';
				// }
				// elseif(!empty($get_customer1)) {
				// 	$role = 'customer';
				// }
				// else {
				// 	$role = 'manager';
				...
				if($update_user)
				{
					...
					if(!empty($get_team_member))
					{
						...
						foreach ($check_b_exist as $member)
						{
							$manageReferre = $this->db->get_where('tbl_manage_referrer',array('referrer_id' => $user_id,'business_id'=>$member['business_id'],'is_del'=>0))->row_array();
							if(empty($manageReferre)) {
								$this->db->select('*');
								$this->db->where('is_del',0);
								$this->db->where('manage_id',$member['manage_id']);
								$this->db->where('info_id',$member['info_id']);
								$this->db->where('find_in_set("'.$user_id.'", team_member_id) <> 0');
								...
								$input = $user_id;
								$list = $member['team_member_id'];
								$array1 = Array($input);
								$array2 = explode(',', $list);
								$array3 = array_diff($array2, $array1);
								$output = implode(',', $array3);

								$this->mdl_common->update_record('tbl_manage_referrer',array('manage_id'=>$member['manage_id'],'is_del'=>0),array('team_member_id'=>$output));
							}
						}
					}
					...
					$user_data = array(
						...
						'is_verify'=>'0',
						'is_profile_complete'=> $user_data['is_profile_complete'],
						'business_country_code' => $user_data['business_country_code'],
						'business_phone_number' => $user_data['business_phone_number'],
						'otp' => "$otp"
					);
	...
	public function create_profile_post()
	{
		...
		$input_data['business_country_code'] = $this->input->post('business_country_code',TRUE);
		$input_data['business_phone_number'] = $this->input->post('business_phone_number',TRUE);
		...
		if($res_validate_token['flag'] > 0)
		{
			...
			if($exist_response[RESPONSE_FLAG])
			{
				if($input_data['business_phone_number'] != '') {
					$this->db->select("*");
					$this->db->where("((business_phone_number = ".$input_data['mobile_no']." OR mobile_no = ".$input_data['business_phone_number']." OR business_phone_number = ".$input_data['business_phone_number']."))");
					$this->db->where('user_id != ', $input_data['user_id']);
					$this->db->where("is_del", 0);
					$userDatas = $this->db->get("tbl_user")->row_array();
				}
				else {
					$this->db->select("*");
					$this->db->where("(business_phone_number = ".$input_data['mobile_no']." OR mobile_no = ".$input_data['mobile_no'].")");
					$this->db->where('user_id != ', $input_data['user_id']);
					$this->db->where("is_del", 0);
					$userDatas = $this->db->get("tbl_user")->row_array();
				}
				if(empty($userDatas)) {
					...
					$user_data['business_country_code'] = $input_data['business_country_code'];
					...
					if($user_data_toekn['first_name'] == '' && $user_data_toekn['last_name'] == '') {
	...
	public function user_activated_post()
	{
		$input_data['user_id'] = $this->input->post('user_id', TRUE);
		$input_data['access_token'] = $this->input->post('access_token', TRUE);
        $input_data['lang'] = ($this->input->post('lang', TRUE)) ? $this->input->post('lang', TRUE) : '0';
		...
		if($res_validate_token['flag'] > 0)
		{
			...
			$response[RESPONSE_FLAG] = RESPONSE_FLAG_SUCCESS;
			$response[RESPONSE_MESSAGE] = _DATA_FOUND; 
			$user_data[ACCESS_TOKEN] = $input_data['access_token'];
			$response[RESPONSE_DATA] = $user_data;
		}
		else
		{
			$response[RESPONSE_FLAG] = $res_validate_token['flag'];
			//$response[RESPONSE_MESSAGE] = $res_validate_token['msg'];
			$response[RESPONSE_MESSAGE] = __USER_NOT_LOGIN;
		} 
        $this->response($response, 200); 
	}
	...
	public function add_new_referral_post()
	{
		...
		$input_data['user_id'] = $this->input->post('user_id',TRUE);
		$input_data['access_token'] = $this->input->post('access_token',TRUE);
		$input_data['relationship'] = $this->input->post('relationship',TRUE);
		...
		$input_data['relationship2'] = $this->input->post('relationship2',TRUE);
		$input_data['referral_mobile_no2'] = $this->input->post('referral_mobile_no2',TRUE);
		$input_data['re_country_code2'] = $this->input->post('re_country_code2',TRUE);
		$input_data['referral_name2'] = $this->input->post('referral_name2',TRUE);
		$input_data['relationship3'] = $this->input->post('relationship3',TRUE);
		$input_data['referral_mobile_no3'] = $this->input->post('referral_mobile_no3',TRUE);
		$input_data['re_country_code3'] = $this->input->post('re_country_code3',TRUE);
		$input_data['referral_name3'] = $this->input->post('referral_name3',TRUE);
		...
		$input_parameter = array('user_id','access_token','referral_mobile_no','business_mobile_no','re_country_code','bu_country_code','message','referral_name','business_name','relationship','relationship2','relationship3','referral_name2','referral_name3','referral_mobile_no2','referral_mobile_no3','re_country_code2','re_country_code3');
		...
		{
			$this->db->select("*");
			$this->db->where("(business_phone_number = ".$input_data['business_mobile_no']." OR mobile_no = ".$input_data['business_mobile_no'].")");
			//$this->db->where("mobile_no",$input_data['business_mobile_no']);
			$this->db->where("is_del", 0);
			$user_data = $this->db->get("tbl_user")->row_array();
			//echo $this->db->last_query();die;
			if(!empty($user_data)) {
				$teamMember = $this->mdl_common->get_record("tbl_manage_team_member", Array('mobile_no' => $input_data['business_mobile_no']))->row_array();
				...
				$this->db->where('referrer_id', $input_data['user_id']);
				$block_data = $this->db->get('tbl_block_referrer')->result_array();
				if(empty($block_data)) {
					if($input_data['business_mobile_no'] != $input_data['referral_mobile_no'] && $input_data['business_mobile_no'] != $input_data['referral_mobile_no2'] && $input_data['business_mobile_no'] != $input_data['referral_mobile_no3'])
					{
						...
						$new_referral_info = array(
							...
							'referral_mobile_no2' => $input_data['referral_mobile_no2'],
							'referral_country_code2' => $input_data['re_country_code2'],
							'referral_name2' => $input_data['referral_name2'],
							'relationship2' => $input_data['relationship2'],
							'referral_mobile_no3' => $input_data['referral_mobile_no3'],
							'referral_country_code3' => $input_data['re_country_code3'],
							'referral_name3' => $input_data['referral_name3'],
							'relationship3' => $input_data['relationship3'],
						...
						if($input_data['referral_mobile_no']!= '')
						{
							...
							$check_business_referral_exist = $this->db->get_where('tbl_user',array('business_phone_number'=>$input_data['referral_mobile_no'],'is_del'=>0))->row_array();
							
							if(!empty($check_referral_exist))
							{
								$referral_id = $check_referral_exist['user_id'];
							}
							elseif(!empty($check_business_referral_exist))
							{
								$referral_id = $check_business_referral_exist['user_id'];
							}
							else
							{
								$referral_id = 0;
						...
						if($input_data['referral_mobile_no2']!= '')
						{
							$check_referral_exist = $this->db->get_where('tbl_user',array('mobile_no'=>$input_data['referral_mobile_no2'],'is_del'=>0))->row_array();

							$check_business_referral_exist = $this->db->get_where('tbl_user',array('business_phone_number'=>$input_data['referral_mobile_no2'],'is_del'=>0))->row_array();
							
							if (!empty($check_referral_exist))
							{
								$referral_id2 = $check_referral_exist['user_id'];
							}
							elseif(!empty($check_business_referral_exist))
							{
								$referral_id2 = $check_business_referral_exist['user_id'];
							}
							else
							{
								$referral_id2 = 0;
						...
						if($input_data['referral_mobile_no3']!= '')
						{
							$check_referral_exist = $this->db->get_where('tbl_user',array('mobile_no'=>$input_data['referral_mobile_no3'],'is_del'=>0))->row_array();

							$check_business_referral_exist = $this->db->get_where('tbl_user',array('business_phone_number'=>$input_data['referral_mobile_no3'],'is_del'=>0))->row_array();
							
							if(!empty($check_referral_exist))
							{
								$referral_id3 = $check_referral_exist['user_id'];
							}
							elseif(!empty($check_business_referral_exist))
							{
								$referral_id3 = $check_business_referral_exist['user_id'];
							}
							else
							{
								$referral_id3 = 0;
						...
						{
							$check_business_exist = $this->db->get_where('tbl_user',array('mobile_no'=>$input_data['business_mobile_no'],'is_del'=>0, 'is_flag' => 1))->row_array();

							$check_business_business_exist = $this->db->get_where('tbl_user',array('business_phone_number'=>$input_data['business_mobile_no'],'is_del'=>0, 'is_flag' => 1))->row_array();
							
							if(!empty($check_business_exist))
							{
								$business_id = $check_business_exist['user_id'];
							}
							elseif(!empty($check_business_business_exist)) 
							{
								$business_id = $check_business_business_exist['user_id'];
							}
							else
							{
								$business_id = 0;
						...
						if($info_id)
						{
							$manage_referral = array(
								...
								'referral_id2' => (@$referral_id2) ? $referral_id2 : 0,
								'referral_id3' => (@$referral_id3) ? $referral_id3 : 0,
							...
							if($manage_referral['referral_id2'] == '0')
							{
								$get_push_msg = $this->db->get_where('tbl_all_notifications',Array('notification_type'=> 'SMS','send_to'=>'CUSTOMER','when_send'=>'NEW_REFERRAL_CREATED','type'=>'SMS_IF_CUSTOMER_IS_NOT_A_REGISTERED_USER'))->row_array();
								$push_msg = str_ireplace('<referrer_name>', $referrer_name, $get_push_msg['message']);
								$r_to = $input_data['re_country_code2'].''.$input_data['referral_mobile_no2'];
								$sms=$this->mdl_common->send_sinch_sms(str_replace(" ","",$r_to),$push_msg);
							}
							else
							{	
								...
								$push_msg = str_ireplace('<referrer_name>', $referrer_name, $get_push_msg['message']);
								$this->db->select("P.user_id,register_id,P.device_type");
								$this->db->where('P.user_id', $manage_referral['referral_id2']);
								$this->db->where('U.is_notification',0);
								$this->db->from("tbl_push AS P");
								$this->db->join('tbl_user AS U','P.user_id=U.user_id','left');
								$this->db->order_by('P.push_id',"DESC");
								...
								if (!empty($register_token)) {
									...
									$n_data = array(
										'user_id' => $manage_referral['referral_id2'],
										'message' => $push_msg,
										'notification_post_user_id'=> $input_data['user_id'],
										'is_flag'=> 3,
										'manage_id'=> $manage_id,
									...
									$notification_count = $this->db->get_where('tbl_notification',Array('is_count' => 0,'user_id' => $manage_referral['referral_id2']))->num_rows();
									$message_array['custom_data']['count'] = $notification_count;
									$message_array['count'] = $notification_count;
									$message_array['custom_data']['notification_id'] = $notification_id;
							...
							if($manage_referral['referral_id3'] == '0')
							{
								$get_push_msg = $this->db->get_where('tbl_all_notifications',Array('notification_type'=> 'SMS','send_to'=>'CUSTOMER','when_send'=>'NEW_REFERRAL_CREATED','type'=>'SMS_IF_CUSTOMER_IS_NOT_A_REGISTERED_USER'))->row_array();
								$push_msg = str_ireplace('<referrer_name>', $referrer_name, $get_push_msg['message']);
								$r_to = $input_data['re_country_code3'].''.$input_data['referral_mobile_no3'];
								$sms=$this->mdl_common->send_sinch_sms(str_replace(" ","",$r_to),$push_msg);
							}
							else
							{	
								...
								$push_msg = str_ireplace('<referrer_name>', $referrer_name, $get_push_msg['message']);
								$this->db->select("P.user_id,register_id,P.device_type");
								$this->db->where('P.user_id', $manage_referral['referral_id3']);
								$this->db->where('U.is_notification',0);
								$this->db->from("tbl_push AS P");
								$this->db->join('tbl_user AS U','P.user_id=U.user_id','left');
								$this->db->order_by('P.push_id',"DESC");
								...
								if (!empty($register_token)) {
									...
									$n_data = array(
										'user_id' => $manage_referral['referral_id3'],
										'message' => $push_msg,
										'notification_post_user_id'=> $input_data['user_id'],
										'is_flag'=> 3,
										'manage_id'=> $manage_id,
									...
									$notification_count = $this->db->get_where('tbl_notification',Array('is_count' => 0,'user_id' => $manage_referral['referral_id3']))->num_rows();
									$message_array['custom_data']['count'] = $notification_count;
									$message_array['count'] = $notification_count;
									$message_array['custom_data']['notification_id'] = $notification_id;
					...
					$response[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
					$response[RESPONSE_MESSAGE] = __YOU_BLOCKED_ANOTHER_USER;
				}
			}
			else {
				$response[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
				$response[RESPONSE_MESSAGE] = __MOBILE_NO_NOT_REGISTER;
			}
		}	
		else
		{
	...
	public function pay_referral_fee_by_business_post()
	{
		...
		if($res_validate_token['flag'] > 0)
		{
			...
			if($role == 'manager') {
				$exist_referrer_data = $this->db->get_where('tbl_manage_referrer',array('manage_id'=>$input_data['manage_id'],'info_id'=>$input_data['info_id'],'business_id' => $input_data['user_id'],'is_del'=>0))->row_array();
				$referrer_id = $exist_referrer_data['referrer_id'];
				if(empty($exist_referrer_data)) {
					$exist_referrer_data = $this->db->get_where('tbl_manage_referrer',array('manage_id'=>$input_data['manage_id'],'info_id'=>$input_data['info_id'],'referrer_id' => $input_data['user_id'],'is_del'=>0))->row_array(); 
					$referrer_id = $exist_referrer_data['business_id'];
				}
			}
			else {
				$exist_referrer_data = $this->db->get_where('tbl_manage_referrer',array('manage_id'=>$input_data['manage_id'],'info_id'=>$input_data['info_id'],'business_id' => $input_data['user_id'],'is_del'=>0))->row_array();
			...
			if(!empty($exist_referrer_data))
			{
				//$get_account_data = $this->db->get_where('tbl_referrer_bank',Array('referrer_id' => $referrer_id,'is_del' => '0'))->row_array();
				...
				{
					if($get_card_data['card_stripe_token'] !='')
					{
						if($exist_referrer_data['business_status'] != 6)
						{
							if($input_data['paid_amount']!= '')
							{
								...
								if($sender_data['role'] == 'manager') {
									$teamMember = $this->mdl_common->get_record('tbl_manage_team_member',Array('user_id' => $input_data['user_id'],'is_del' => 0))->row_array();
								}
								else {
									$teamMember = $this->mdl_common->get_record('tbl_manage_team_member',Array('mobile_no' => $sender_data['mobile_no'],'is_del' => 0))->row_array();
								}

								$monthly_referral_fee = '';
								if(!empty($teamMember)) {
								...
								$this->db->where('user_id', $input_data['user_id']);
								$this->db->where('manage_id', $input_data['manage_id']);
								$this->db->where('info_id', $input_data['info_id']);
								//$this->db->where('is_paid', 1);
								$total_sum = $this->db->get('tbl_payment')->row()->total_sum;
								$total_amount = $total_sum + $input_data['paid_amount'];

								$current_month = date('m', strtotime(date('Y-m-d')));
								//, 'manage_id' => $input_data['manage_id'],'info_id'=>$input_data['info_id']
								$paymentData = $this->mdl_common->get_record('tbl_payment',Array('user_id' => $input_data['user_id']))->row_array();
								$payment_date = date('m', strtotime($paymentData['payment_date']));
								...
								if(empty($paymentData)) {
									...
									if($charge_id != '')
									{
										...
										$this->mdl_common->update_record('tbl_manage_referrer',Array('manage_id'=>$input_data['manage_id'],'info_id'=>$input_data['info_id']),$up_status_data);
									...
									if($monthly_referral_fee == '') {
										try
										{
											if(strtotime($current_month) == strtotime($payment_date)) {
												// for manage db
												$amt = $price = $input_data['paid_amount'];
												$fees = ((($price * STRIPE_FEE) / 100) + EXTRA_STRIPE_FEE);
												$change_amount = $amt - $fees;
										...
										if($charge_id != '')
										{
											...
											$ins_rdata['payment_date'] = date('Y-m-d');
											$ins_rdata['date_added'] = date('Y-m-d H:i:s');
											$ins_rdata['application_fee'] = ($current_month == $payment_date) ? 0 : $application_fee;
											$this->mdl_common->insert_record('tbl_payment',$ins_rdata);
					...
					else
					{
						...
						$business_name = $business_data['first_name'].' '.$business_data['last_name'];
						//$push_msg =  $business_name." has viewed your Referral.";
						$push_msg="Please update your debit card details to receive payment from".$business_name;
	...
	public function add_card_post()
	{
        ...
        $input_data['default_amount'] = $this->input->post('default_amount', TRUE);
        $input_data['is_auto_payment'] = $this->input->post('is_auto_payment', TRUE);
        ...
        if($res_validate_token['flag'] > 0)
        {
			...
			{   
				...
				$in_data['default_amount'] = ($input_data['default_amount'] != '') ? $input_data['default_amount'] : 0;
				$in_data['is_auto_payment'] = ($input_data['is_auto_payment'] != '') ? $input_data['is_auto_payment'] : 0;
				$in_data['card_type'] = $card_type;
			...
			{  
				...
				$in_data['default_amount'] = ($input_data['default_amount'] != '') ? $input_data['default_amount'] : 0;
				$in_data['is_auto_payment'] = ($input_data['is_auto_payment'] != '') ? $input_data['is_auto_payment'] : 0;
				$uptoken_data['card_type'] = $card_type;
        	...
        	if(empty($account_exist))
        	{
	            try
				{
					...
					$account = \Stripe\Account::create([
						...
						"business_profile[url]" => 'www.jointlyapp.net',
	...
	function country_list_post()
	{
		...
		$this->db->select('country_id,country_name as name,country_code as dial_code,c_sort_name as code,c_sort_name');
	...
	function check_mobile_no_post()
    {
		...
		$input_parameter = array('user_id','access_token','mobile_no');
        ...
        if($res_validate_token['flag'] > 0)
        {
			$userData = $this->mdl_common->get_record("tbl_user", Array('mobile_no' => $input_data['mobile_no'], 'is_del' => '0'))->row_array();
			if(empty($userData)) {
				$response[RESPONSE_FLAG] = RESPONSE_FLAG_SUCCESS;
				$response[RESPONSE_MESSAGE] = _MOBILE_NO_NOT_REGISTER;
			}
			else {
				$response[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
				$response[RESPONSE_MESSAGE] = __MOBILE_NO_ALREADY_REGISTER;
			}
		}
        else
        {
	...
	function manage_tema_member_post()
    {
		...
		$input_parameter = array('user_id','access_token','mobile_array','subscription_level');
        ...
        if($res_validate_token['flag'] > 0)
        {
			...
			if(!empty(json_decode($mobile_array))) 
			{
				if($input_data['subscription_level'] == 'Bronze') {
					$subscription_level = 1;
				}
				elseif($input_data['subscription_level'] == 'Silver') {
				...
				foreach(json_decode($mobile_array) as $key => $value) {
					...
					if(empty($team_member)) {
						...
						$ins_data['subscription_level'] = $subscription_level;
						...
						$get_push_msg = $this->db->get_where('tbl_all_notifications',Array('notification_type'=> 'SMS','send_to'=>'AGENCY_TEAM_MEMBER','when_send'=>'AGENCY_MANAGER_CREATE_AGENCY_TEAM_MEMBER','type'=>'PUSH_NOTIFICATION_OR_SMS_IF_AGENCY_TEAM_MEMBER'))->row_array();
						$push_msg = str_ireplace('', '', $get_push_msg['message']);
						$r_to = $country_code.''.$mobile_no;
						$sms=$this->mdl_common->send_sinch_sms(str_replace(" ","",$r_to),$push_msg);
					}
					else {
						$up_data['country_code'] = $country_code;
						$up_data['mobile_no'] = $mobile_no;
						$up_data['subscription_level'] = $subscription_level;
				...
				$this->db->where('user_id', $input_data['user_id']);
				$this->db->where_in('mobile_no  NOT', $mobile_number);
				$this->db->delete('tbl_manage_team_member');
				...
				$this->mdl_common->delete_record('tbl_manage_team_member',array('user_id' => $input_data['user_id']));
				if($input_data['subscription_level'] == 'Bronze') {
					$subscription_level = 1;
				}
				elseif($input_data['subscription_level'] == 'Silver') {
	...
	function tema_member_list_post() 
	{
		...
		{
			$userData =  $this->db->get_where('tbl_user',array('user_id' => $input_data['user_id'],'is_del' => 0))->row_array();
			if($userData['subscription_level'] == 1) {
				$subscription_level = 'Bronze';
			}
			elseif($userData['subscription_level'] == 2) {
				$subscription_level = 'Silver';
			}
			else {
				$subscription_level = 'Gold';
			}
			$max_number_tier_level = $userData['max_number_tier_level'];

			$member_list = $this->mdl_common->get_record('tbl_manage_team_member', Array('user_id' => $input_data['user_id'], 'is_del' => 0))->result_array();
			$monthly_referral_fee = '';
			if(!empty($member_list))
			{
				...
				foreach($member_list as $row) {
					...
					$monthly_referral_fee = $row['monthly_referral_fee'];
					$user = $this->mdl_common->get_record('tbl_user', Array('mobile_no' => $mobile_no))->row_array();
					if($row['country_code'] != '' && $row['mobile_no'] != ''){
						$resultArray[] = Array(
				...
				$response['max_number_tier_level'] = $max_number_tier_level;
				$response['monthly_referral_fee'] = $monthly_referral_fee;
	...
	function relationship_list_post() 
    {
        ...
        if (!empty($relationship)) {	
			$user_data = $this->mdl_common->get_record('tbl_user', Array('user_id' => $input_data['user_id'], 'is_del' => 0))->row_array();
			...
			$inviteCustomer = $this->mdl_common->get_record('tbl_invite_customer', Array('mobile_no' => $user_data['mobile_no'], 'is_del' => 0))->row_array();
			if(!empty($inviteCustomer)) {
				$userData = $this->mdl_common->get_record('tbl_user', Array('user_id' => $inviteCustomer['user_id'], 'is_del' => 0))->row_array();
			...
			$teamMember = $this->mdl_common->get_record('tbl_manage_team_member', Array('mobile_no' => $user_data['mobile_no'], 'is_del' => 0))->row_array();
			if(!empty($teamMember)) {
				$userData1 = $this->mdl_common->get_record('tbl_user', Array('user_id' => $teamMember['user_id'], 'is_del' => 0))->row_array();
				$country_code = $userData1['country_code'];
				$mobile_no = $userData1['mobile_no'];
				$name = $userData1['first_name'].''.$userData1['last_name'];
				$business_country_code = $userData1['business_country_code'];
				$business_phone_number = $userData1['business_phone_number'];
			}

			$agencyData = (object)[];
			if(!empty($user_data)) {
				$agencyData = Array(
					'country_code' => ($business_country_code != '') ? $business_country_code : $country_code,
					'mobile_no' => ($business_phone_number != '') ? $business_phone_number : $mobile_no,
					'name' => $name,
				);
			}
            ...
            $response['agency_data'] = $agencyData;
        } else {
            $response[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
            $response[RESPONSE_MESSAGE] = __SOMETHING_WENT_WRONG;
        }
	...
	function invite_customer_post()
	{
		...
		$input_parameter = array('user_id','access_token','country_code','mobile_no');
        ...
        if($res_validate_token['flag'] > 0)
        {
			$invitCustomer = $this->mdl_common->get_record('tbl_invite_customer', Array('mobile_no' => $input_data['mobile_no'], 'is_del' => 0))->row_array();
			if(empty($invitCustomer)) {
				$this->db->select("*");
				$this->db->where("(business_phone_number = ".$input_data['mobile_no']." OR mobile_no = ".$input_data['mobile_no'].")");
				$this->db->where("is_del", 0);
				$userData = $this->db->get("tbl_user")->row_array();

				if(empty($userData)) {
					$ins_data['user_id'] = $input_data['user_id'];
					$ins_data['country_code'] = $input_data['country_code'];
					$ins_data['mobile_no'] = $input_data['mobile_no'];
					$invite_customer = $this->mdl_common->insert_record('tbl_invite_customer',$ins_data);

					$user_data = $this->mdl_common->get_record('tbl_user', Array('user_id' => $input_data['user_id']))->row_array();
					$user_name = $user_data['first_name'].' '.$user_data['last_name'];

					$get_push_msg = $this->db->get_where('tbl_all_notifications',Array('notification_type'=> 'SMS','send_to'=>'CUSTOMER','when_send'=>'NEW_REFERRAL_CREATED','type'=>'SMS_IF_CUSTOMER_IS_NOT_A_REGISTERED_USER'))->row_array();
					$push_msg = str_ireplace('<referrer_name>', $user_name, $get_push_msg['message']);
					$r_to = $input_data['country_code'].''.$input_data['mobile_no'];
					$sms = $this->mdl_common->send_sinch_sms(str_replace(" ","",$r_to),$push_msg);

					if($invite_customer)
					{
						$response[RESPONSE_FLAG] = RESPONSE_FLAG_SUCCESS;
						$response[RESPONSE_MESSAGE] = _INVITE_CUSTOMER;
					}
					else
					{
						$response[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
				...
				$response[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
				$response[RESPONSE_MESSAGE] = __CUSTOMER_ALERADY_INVITED;
			}
		}
        else
        {
	...
	function block_referrer_post()
	{
        ...
        if($res_validate_token['flag'] > 0)
        {
			$referrer_id = $this->mdl_common->get_record('tbl_user', Array('mobile_no' => $input_data['mobile_no']))->row()->user_id;

			$ins_data['user_id'] = $input_data['user_id'];
			$ins_data['referrer_id'] = ($referrer_id) ? $referrer_id : '';
			$ins_data['mobile_no'] = $input_data['mobile_no'];
			$block_referrer = $this->mdl_common->insert_record('tbl_block_referrer',$ins_data);

			if($block_referrer)
			{
				$response[RESPONSE_FLAG] = RESPONSE_FLAG_SUCCESS;
				$response[RESPONSE_MESSAGE] = 	_BLOCK_REFERRER;
			}
			else
			{
				$response[RESPONSE_FLAG] = RESPONSE_FLAG_FAIL;
