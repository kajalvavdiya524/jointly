<?php
require APPPATH.'libraries/Twilio/autoload.php'; // Loads the library
...
class Mdl_common extends CI_Model
{
	...
	function send_sinch_sms($to, $message)
	{
		...
		try
		{
			...
			//'From'=> "+61437082080",
			"messagingServiceSid" => "MG9127fde90a5bbe893237944598bfb4d0",
			"Body"=> $message,
			));
			
			// echo "<pre>";
			// print_r($messages);
			// echo $messages->sid;die;
		...
		{
			$error = $e->getMessage(); //print_r($error);die;
		}
	}
	...
	{
		return strtoupper(substr(uniqid(mt_rand(00000000,999999999), true), 0, $l));
	}
	function generate_random_id($l = 4)
	{
		return strtoupper(substr(uniqid(mt_rand(00000000,999999999), true), 0, $l));
	}
